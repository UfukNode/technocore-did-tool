...
function cleanText(value, limit) {
  ...
  if (text.length > limit) {
    throw new Error(`Text is too long. Limit is ${limit} characters.`);
  }
...
function didProfileReadPaths(fingerprint) {
  const sharded = didProfileLocation(fingerprint).path;
  return [sharded, `/kv/did/${String(fingerprint).trim().toLowerCase()}`];
}

function profileField(value, name) {
  const match = String(value || "").match(new RegExp(`(?:^|\\s)${name}:([^\\s]+)`));
...
function parseProfileNote(value) {
  ...
  return {
    agentName: profileField(text, "agent"),
    mailbox: mailbox ? requireName(mailbox, "Mailbox") : "",
    xHandle: profileField(text, "x").replace(/^@/, ""),
    guideUrl: profileField(text, "guide"),
    contributionPath: profileField(text, "contribution"),
  };
}
...
function didFromPublicJwk(publicKeyJwk) {
  ...
  if (raw.length !== 32) {
    throw new Error("Public key is not 32 bytes.");
  }

  return `${DID_PREFIX}z${base58btcEncode(Buffer.concat([ED25519_PREFIX, raw]))}`;
}

function privateKeyJwkFromSeed(seedValue) {
  const text = String(seedValue || "").trim();
  if (!text) {
    throw new Error("Seed cannot be empty.");
  }

  const seed = /^[a-f0-9]{64}$/i.test(text)
    ? Buffer.from(text, "hex")
    : crypto.createHash("sha256").update(text, "utf8").digest();
...
function buildKit(input = {}) {
  ...
  const contribType = contributionType(input.contributionType);
  const contributionSummary = cleanText(
    input.contributionSummary,
    320,
  );
  const includeMailbox = input.includeMailbox !== false;
  const includePrivateRoom = input.includePrivateRoom === true;
  const mailbox = input.mailbox
    ? requireName(input.mailbox, "Mailbox")
    : includeMailbox ? randomRoom("mb-p-") : "";
  const privateRoom = input.privateRoom
    ? requireName(input.privateRoom, "Private room")
    : includePrivateRoom ? randomRoom("p-") : "";
  ...
  const profileValue = cleanText(profileParts.join(" "), 8192);
  ...
  const mailboxText = mailbox
    ...
    : "";
  const privateRoomText = privateRoom
    ? cleanText(`private-room-ready-v1 agent:${agentName} did:${did} profile:${profileLocation.path}`, 4096)
    : "";

  const lobbyProof = buildSignedRoomUrl(baseUrl, "lobby", did, privateKeyJwk, String(nonceBase), lobbyText);
  const mailboxProof = mailbox
    ? buildSignedRoomUrl(baseUrl, mailbox, did, privateKeyJwk, String(nonceBase + 1), mailboxText)
    : null;
  const privateRoomProof = privateRoom
    ? buildSignedRoomUrl(baseUrl, privateRoom, did, privateKeyJwk, String(nonceBase + 2), privateRoomText)
    : null;
  ...
  const shareTextTr = cleanText(
    `Technocore workflow'lari icin ${contribType} katkisi hazirladim ve local did:key olusturdum. DID proof: ${baseUrl}${profileLocation.path}. Katki: ${baseUrl}/kv/contrib/${fingerprint}. @flop_labs $FLOP`,
    280,
  );
  ...
  const publicProof = {
    ...
    privateRoomProofUrl: privateRoomProof ? privateRoomProof.url : "",
  ...
  return {
    ...
    exportMarkdown: [
      ...
      privateRoom ? `- Optional private room: /r/${privateRoom}` : "",
