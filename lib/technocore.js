...
function fingerprintOfDid(did) {
  return crypto.createHash("sha256").update(did, "utf8").digest("hex").slice(0, 32);
}

function didProfileLocation(fingerprint) {
  const text = String(fingerprint || "").trim().toLowerCase();
  if (!/^[a-f0-9]{32}$/.test(text)) {
    throw new Error("Fingerprint must be 32 lowercase hex characters.");
  }
...
function noteField(value, name, nextNames) {
  const keys = nextNames.map((item) => `${item}:`).join("|");
  const match = String(value || "").match(new RegExp(`(?:^|\\s)${name}:(.*?)(?=\\s(?:${keys})|$)`));
...
function parseContributionNote(value) {
  const text = String(value || "");
  return {
...
function buildKit(input = {}) {
  ...
  const technocoreProof = buildSignedRoomUrl(baseUrl, "technocore", did, privateKeyJwk, String(nonceBase + 1), technocoreText);
  ...
  return {
    ...
    exportMarkdown: [
      ...
      `- Technocore contribution announcement: ${technocoreProof.url}`,
